...
msdidlist = []
for msd_id in list(scores.keys()):
    with tables.open_file(msd_id_to_h5(msd_id)) as h5:
        aterms = list(h5.root.metadata.artist_terms)[:5]

        aweights = list(h5.root.metadata.artist_terms_weight)[:5]
        for i, aterm in enumerate(aterms):
            if aterm.decode("utf-8") in 'synthpop':
                if aweights[i] > .94:
                    msdidlist.append(msd_id)
print(scores[msd_id])

print(msdidlist)        
print(scores[msd_id])
midimd5list = []
for msdid in msdidlist:
    m5list = []
    for midi_md5, score in scores[msdid].items():
        m5list.append(midi_md5)
    midimd5list.append(m5list)
midipathlist = []
for i, msdid in enumerate(msdidlist):
    for midi_md5 in midimd5list[i]:
        midpath = get_midi_path(msdid, midi_md5, 'matched')
        midipathlist.append(midpath)
print(midipathlist)
